...
percenterApp.controller("modifyGoodsCtrl", ["$scope", "$rootScope", "$routeParams", "Http", "scopeService", function($scope, $rootScope, $routeParams, Http, scopeService) {
    ...
    var object2 = {},
        ...
        goodsInfo, storeInfo, val, valArr, $this, hasContain, index, type, $el, $li, $rule, id, hasPackage;
                ...
                Http.request({
                    ...
                    success: function(res) {
                        if(res.resultCode === "1") {
                            ...
                            $.each(mainPicList, function(index, pic) {
                                $("#picType2 input[name='uploadfile']").eq(index).parent(".upimg_bg").append("<img class='uploaded-pic' src='" + pic.picsUrl + "' />");
                            });
                            ...
                            try {
                                modifyProdRichEditor.html(goodsInfo.url);
                            } catch(e) {
    ...
    $scope.submit = function() {
        ...
        //详情富文本
        if (tip === "" && (modifyProdRichEditor.html()==null || modifyProdRichEditor.html()=="")) {
            tip = "请编辑产品详情！";
        }
        ...
        post.url = modifyProdRichEditor.html();

        post.prodId = goodsInfo.prodId;
        post.providId = goodsInfo.providId;
        post.groupId = goodsInfo.groupId;
        post.shelfId = goodsInfo.shelfId;
        post.groupName = storeInfo.groupName;
        post.templeType = storeInfo.templeType;

        Http.request({
            url: config.editProduct,
            method: "POST",
            data: {
    ...
    $scope.preview = function () {
        ...
        post.prodTypeCd = $("#prodTypeCd").val();
        post.prodTypeCdName = "";
        if (post.prodTypeCd != 0) {
            post.prodTypeCdName = $("option[value='" + post.prodTypeCd + "']").val();
        }
        post.description = $scope.description;
        post.transcat = transcatMap[$("#transcat").val()];
        post.downloadUrl = $scope.downloadUrl;
     ...
     * 添加一条新规则
     */
    $(document).on("click", ".add-one-rule", function(e) {
     ...
     * 删除一条新规则
     */
    $(document).on("click", ".remove-one-rule", function(e) {
     ...
     * 类型限制
     */
    $(document).on("blur", "input[data-limit]", function(e) {
        $this = $(this);
     ...
     * 删除
     */
    $(document).on("click", "a.delete-type", function(e) {
     ...
     * 隐藏
     */
    $(document).on("click", "span.hide-btn", function(e) {
     ...
     * 显示
     */
    $(document).on("click", "span.open-btn", function(e) {
     ...
     * 标题区域控制显示/隐藏
     */
    $(document).on("click", "div.type-tiles-title", function(e) {
        $el = $(e.target).closest(".type-item");
...
var modifyProdRichEditor = null;

$(function(){

    KindEditor.ready(function (K) {
        modifyProdRichEditor = K.create('#modifyProdRichEditor', {
